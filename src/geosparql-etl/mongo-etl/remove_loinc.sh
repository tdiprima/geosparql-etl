...
# Script to remove loinc prefix line from .ttl.gz files in parallel
# Usage: ./process_ttl_files.sh
...
# Export the function and variables so parallel processes can use them
export -f process_file
export PREFIX_TO_REMOVE

# Find all .ttl.gz files and process them in parallel
find "${DATA_DIR}" -name "*.ttl.gz" -type f | \
